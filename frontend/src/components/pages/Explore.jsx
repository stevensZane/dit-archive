...
const Explore = () => {
  ...
  const allFilieres = [
    "Niveau d'études",
    ...new Set(projects.map((p) => p.level).filter(Boolean)),
  ];
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="max-w-2xl">
              <h1 className="flex items-center gap-3 text-4xl font-black text-gray-900 mb-4 italic uppercase tracking-tighter">
                ...
                <img
                  src="/DIT ARCHIVE mascot(1).svg"
                  alt="dit archive mascot"
                  className="h-20 w-auto"
                />
              </h1>
              <p className="text-gray-500 font-medium">
                Explorez tous les projets archivés : du code source aux archives
                serveur.
              </p>
            </div>
            ...
            <div className="relative w-full md:w-96">
              ...
              {isSearching && (
                <Loader2
                  className="absolute right-4 top-1/2 -translate-y-1/2 animate-spin text-[#E91E63]"
                  size={20}
                />
              )}
            </div>
          ...
          {loading && projects.length === 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3, 4, 5, 6].map((n) => (
                <div
                  key={n}
                  className="h-64 bg-gray-100 animate-pulse rounded-[32px]"
                ></div>
              ))}
            </div>
