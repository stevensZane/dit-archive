        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-3xl p-8 md:p-10 shadow-sm border border-gray-100">
              ...
              <div className="mt-10 pt-8 border-t border-slate-50">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <Code size={16} /> Technologies & Stack
                </h3>
                <div className="flex flex-wrap gap-2">
                  {project.primary_language && (
                    <span className="bg-[#004751] text-white px-4 py-1.5 rounded-xl text-sm font-bold">
                      #{project.primary_language.toUpperCase()}
                    </span>
                  )}
                  {project.technologies_list &&
                    project.technologies_list
                      .split(",")
                      .map(t => t.trim())
                      .filter((lang) => lang.toLowerCase() !== project.primary_language?.toLowerCase())
                      .map((lang, index) => (
                        <span key={`lang-${index}`} className="bg-[#669099]/10 text-[#004751] border border-[#669099]/20 px-4 py-1.5 rounded-xl text-sm font-black">
                          #{lang.toUpperCase()} {/* 🟢 Rendu en hashtag stylé aux couleurs de ton thème */}
                        </span>
                      ))}
                </div>
              </div>
